#!/usr/bin/env python3
"""Tier-A v1.2b train/eval — iterate FAIL v1.2 (stronger +120h weight).

Leonard TIER_A_V1_2_CALL.md: v1.2 FAIL (val +120h / n_eligible_saves=0).
v1.2b: same ElevCondResidualUNet (base=16, dropout=0.20); w_120=2.0; patience=80.
PASS LOCKED: n_eligible_saves >= 1 required; unconstrained fallback => auto FAIL.
FCN3 frozen. Writes ONLY under runs/phase0/tier_a/v1_2b/.
Does NOT overwrite v0/, v0_1/, v1/, v1_1/, v1_1_expand/, v1_2/, g0_verifying, tier0_holdout.
...
Usage (from ~/fourcastnet):
  ...
  ~/fcn3-venv/bin/python code/tier_a/v1_2b/train.py --config configs/tier_a_v1_2b.yaml --train --allow-gpu --epochs 400
  ~/fcn3-venv/bin/python code/tier_a/v1_2b/train.py --config configs/tier_a_v1_2b.yaml --eval-only \
      --pairs-dir runs/phase0/tier0_holdout_expand/pairs \
      --out-dir runs/phase0/tier_a/v1_2b \
      --ckpt runs/phase0/tier_a/v1_2b/best_residual.pt
"""
...
ROOT = HERE.parents[2]  # ~/fourcastnet  (code/tier_a/v1_2b/)
...
def artifact_stem(cfg: dict) -> str:
    return str(cfg.get("artifact_stem") or "tier_a_v1_2b")
...
def gate_pass_v1(
    ...
    if reload_kind == "fallback_unconstrained_120h" or n_elig < 1:
        eligible_ok = False
...
def assert_no_overwrite(out_dir: Path, cfg: dict) -> None:
    ...
    allowed = expand("~/fourcastnet/runs/phase0/tier_a/v1_2b").resolve()
    if not (out_r == allowed or str(out_r).startswith(str(allowed) + "/")):
        raise RuntimeError(
            f"v1.2b must write under runs/phase0/tier_a/v1_2b/ (got {out_r})"
        )
    for p in [
        expand("~/fourcastnet/runs/phase0/g0/g0_verifying_results.json"),
        expand("~/fourcastnet/runs/phase0/tier0_holdout/tier0_holdout_metrics.json"),
...
def main() -> int:
    ...
    ap.add_argument("--config", default="configs/tier_a_v1_2b.yaml")
    ...
    ap.add_argument(
        "--out-dir",
        default=None,
        help="Override cfg output_dir (e.g. runs/phase0/tier_a/v1_2b)",
    )
    ap.add_argument(
        "--ckpt",
        default=None,
    ...
    report: dict[str, Any] = {
        "schema": cfg.get("schema", "tier_a_v1_2b_results/v1"),
        "created_utc": utc_now(),
        "script": "code/tier_a/v1_2b/train.py",
        "config": str(cfg_path),
    ...
    if mode == "eval_only":
        ckpt_path = expand(
            args.ckpt
            or "~/fourcastnet/runs/phase0/tier_a/v1_2b/best_residual.pt"
        )
