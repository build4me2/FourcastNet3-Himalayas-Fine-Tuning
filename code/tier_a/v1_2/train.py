#!/usr/bin/env python3
"""Tier-A v1.2 train/eval — smaller/regularized elev-conditioned residual UNet.

Leonard TIER_A_V1_CALL.md: v1 FAIL (test overfit). Scaffold v1.2 from v1.1 (expand pairs/bars). Pending Manisha train confirm. NO diffusion.
FCN3 frozen. Writes ONLY under runs/phase0/tier_a/v1_2/.
Does NOT overwrite v0/, v0_1/, v1/, v1_1/, v1_1_expand/, g0_verifying, tier0_holdout.
...
Usage (from ~/fourcastnet):
  ...
  ~/fcn3-venv/bin/python code/tier_a/v1_2/train.py --config configs/tier_a_v1_2.yaml --train --epochs 400
  ~/fcn3-venv/bin/python code/tier_a/v1_2/train.py --config configs/tier_a_v1_2.yaml --eval-only \
      --pairs-dir runs/phase0/tier0_holdout_expand/pairs \
      --out-dir runs/phase0/tier_a/v1_2 \
      --ckpt runs/phase0/tier_a/v1_2/best_residual.pt
"""
...
ROOT = HERE.parents[2]  # ~/fourcastnet  (code/tier_a/v1_2/)
...
def artifact_stem(cfg: dict) -> str:
    return str(cfg.get("artifact_stem") or "tier_a_v1_2")
...
def lead_weights(cfg: dict) -> dict[int, float]:
    """Per-lead loss weights. w_120 MUST be ≥ 1 (Leonard v1.2 / expand bars / v1-reg freeze)."""
    t = cfg.get("train", {})
    ...
    if w120 < 1.0 - 1e-12:
        raise RuntimeError(f"w_120 must be >= 1 (got {w120}); Leonard HOLDOUT_EXPAND / v1.2")
...
THIN_BEAT_THIS = {
    ...
    "val_120h_rule": (
        "val +120h t2m RMSE <= raw (2.219971 K); hard gate for v1.2; "
...
def gate_pass_v1(
    ...
    return {
        ...
        "note": (
            f"v1.2 INTERIM PASS requires (1) val pooled < {vbar} AND "
...
def assert_no_overwrite(out_dir: Path, cfg: dict) -> None:
    ...
    allowed = expand("~/fourcastnet/runs/phase0/tier_a/v1_2").resolve()
    if not (out_r == allowed or str(out_r).startswith(str(allowed) + "/")):
        raise RuntimeError(
            f"v1.2 must write under runs/phase0/tier_a/v1_2/ (got {out_r})"
        )
    for p in [
        expand("~/fourcastnet/runs/phase0/g0/g0_verifying_results.json"),
        expand("~/fourcastnet/runs/phase0/tier0_holdout/tier0_holdout_metrics.json"),
...
def main() -> int:
    ...
    ap.add_argument("--config", default="configs/tier_a_v1_2.yaml")
    ...
    ap.add_argument(
        "--out-dir",
        default=None,
        help="Override cfg output_dir (e.g. runs/phase0/tier_a/v1_2)",
    )
    ap.add_argument(
        "--ckpt",
        default=None,
        help="Frozen residual ckpt for --eval-only (default: v1_2/best_residual.pt)",
    )
    args = ap.parse_args()
    ...
    report: dict[str, Any] = {
        "schema": cfg.get("schema", "tier_a_v1_2_results/v1"),
        "created_utc": utc_now(),
        "script": "code/tier_a/v1_2/train.py",
        "config": str(cfg_path),
        "method": cfg.get("method"),
        "tier": "A",
        "variant": cfg.get("variant"),
        "cut": cfg.get("cut", "v1.2-reg"),
        "diffusion": False,
        "fcn3_weights": "FROZEN",
        ...
        "honesty": cfg.get("notes", {}).get(
            "honesty",
            (
                "Tier-A v1.2 scaffold (same arch as v1.1) on expand pairs; pending Manisha train confirm. "
    ...
    print(
        f"Tier-A {cfg.get('variant', '?')} cut={cfg.get('cut', 'v1.2-reg')} diffusion=NO "
    ...
    if mode == "eval_only":
        ckpt_path = expand(
            args.ckpt
            or "~/fourcastnet/runs/phase0/tier_a/v1_2/best_residual.pt"
        )
