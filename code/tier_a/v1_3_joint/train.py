#!/usr/bin/env python3
"""
Tier-A v1.3 joint winds+t2m train/eval — Leonard TIER_A_V1_3_JOINT_RECIPE.md.

Thin copy of v1_3_joint ElevCondResidualUNet with channel_weights=[1.5,1.5,1.5].
FCN3 frozen. Writes ONLY under runs/phase0/tier_a/v1_3_joint/.
Does NOT overwrite living residual v1_2b_thick2_train/best_residual.pt.
Does NOT touch ERA5 PID 611595 / FCN3 weights.
NO --train until Leonard ACK on living_wind_baseline.json.
...
Usage (from ~/fourcastnet):
  ...
  # after Leonard ACK only:
  ~/fourcastnet/venv/bin/python code/tier_a/v1_3_joint/train.py --config configs/tier_a_v1_3_joint.yaml --train --allow-gpu --epochs 400
"""
...
ROOT = HERE.parents[2]  # ~/fourcastnet  (code/tier_a/v1_2b_thick2_train/)
...
def artifact_stem(cfg: dict) -> str:
    return str(cfg.get("artifact_stem") or "tier_a_v1_3_joint")
...
def lead_weights(cfg: dict) -> dict[int, float]:
    """Per-lead loss weights. w_120 MUST be ≥ 1 (Leonard v1.2b / expand bars)."""
    t = cfg.get("train", {})
    ...
    if w120 < 1.0 - 1e-12:
        raise RuntimeError(f"w_120 must be >= 1 (got {w120}); Leonard HOLDOUT_EXPAND / v1.2b")
...
THIN_BEAT_THIS = {
    ...
    "val_120h_rule": (
        "val +120h t2m RMSE <= raw (2.219971 K); hard gate historical thin; "
...
def uses_expand_bars(cfg: dict) -> bool:
    ...
    if he is True or str(he or "").strip().lower() in {"v1", "v2", "true", "1", "2", "yes"}:
        return True
    variant = str(cfg.get("variant") or "").lower()
    stem = str(cfg.get("artifact_stem") or "").lower()
    out = str(cfg.get("output_dir") or "").lower()
    if "expand" in variant or "expand" in stem or "v1_1_expand" in out or "v1_2" in out or str(variant).startswith("v1_2"):
        return True
...
def beat_this_echo(cfg: dict) -> dict:
    ...
    return {
        ...
        "g0_adapter": g0,
        "frozen_exact": dict(defaults.get("frozen_exact") or {}),
        "bars_set": (
            str(cfg.get("bars_set") or (cfg.get("beat_this") or {}).get("bars_set") or "")
            or ("thick2" if str(cfg.get("holdout_expand") or "").strip().lower() in {"v2", "2"} else None)
            or ("expand" if uses_expand_bars(cfg) else "thin_historical")
        ),
    }
...
def gate_pass_v1(
    ...
    raw_120: float,
    bars: dict,
    n_eligible_saves: int = 0,
    reload_kind: str | None = None,
) -> dict:
    """Leonard v1.2b PASS lock: numeric gates AND n_eligible_saves >= 1.

    Unconstrained +120h fallback => automatic FAIL even if numbers look close.
    """
    ...
    n_elig = int(n_eligible_saves or 0)
    if reload_kind == "eval_only_frozen":
        eligible_ok = True  # frozen ckpt already composite-eligible at train time
    else:
        eligible_ok = n_elig >= 1 and reload_kind == "composite_eligible"
    # Explicit auto-FAIL on unconstrained fallback (Leonard TIER_A_V1_2_CALL.md)
    # Zero-shot eval_only_frozen is exempt from n_elig check (no train loop this run).
    if reload_kind == "fallback_unconstrained_120h":
        eligible_ok = False
    elif reload_kind != "eval_only_frozen" and n_elig < 1:
        eligible_ok = False
    interim = bool(val_ok and test_ok and lead120_ok and eligible_ok)
    return {
        "val_pass": bool(val_ok),
        "test_pass": bool(test_ok),
        "val_120h_le_raw": bool(lead120_ok),
        "n_eligible_saves": n_elig,
        "eligible_ckpt_required": True,
        ...
        "unconstrained_fallback_is_auto_fail": True,
        "note": (
            f"v1.2b INTERIM PASS requires (1) val pooled < {vbar} AND "
            f"(2) test pooled < {tbar} AND (3) val +120h t2m RMSE <= raw "
            f"({raw_120:.6f} K) AND (4) n_eligible_saves >= 1 with "
            "composite_eligible reload. Unconstrained +120h fallback => "
            "automatic FAIL (Leonard TIER_A_V1_2_CALL.md). claim_level "
            "remains interim_era5; g1_claimable=false. "
            "v1_2/ is frozen FAIL (do not overwrite). "
...
def freeze_snapshot() -> dict:
    paths = {
        ...
        "tier_a_v1_1_results": expand("~/fourcastnet/runs/phase0/tier_a/v1_1/tier_a_v1_1_results.json"),
        "tier_a_v1_2_results": expand("~/fourcastnet/runs/phase0/tier_a/v1_2/tier_a_v1_2_results.json"),
...
def assert_no_overwrite(out_dir: Path, cfg: dict) -> None:
    """v1.3 joint writes ONLY under v1_3_joint/; never living residual."""
    ...
    forbidden = [
        ...
        expand("~/fourcastnet/runs/phase0/tier_a/v1_diff"),
    ]
    for p in forbidden:
        pr = p.resolve()
        if out_r == pr or str(out_r).startswith(str(pr) + "/"):
            raise RuntimeError(f"refusing to write into frozen path {p}")
    allowed = expand("~/fourcastnet/runs/phase0/tier_a/v1_3_joint").resolve()
    if not (out_r == allowed or str(out_r).startswith(str(allowed) + "/")):
        raise RuntimeError(
            f"v1.3 joint must write under runs/phase0/tier_a/v1_3_joint/ (got {out_r})"
        )
    for p in [
        expand("~/fourcastnet/runs/phase0/g0/g0_verifying_results.json"),
        expand("~/fourcastnet/runs/phase0/tier0_holdout/tier0_holdout_metrics.json"),
        expand("~/fourcastnet/runs/phase0/tier_a/v1/tier_a_v1_results.json"),
        expand("~/fourcastnet/runs/phase0/tier_a/v1_1/best_residual.pt"),
        expand("~/fourcastnet/runs/phase0/tier_a/v1_2b/best_residual.pt"),
    ]:
        if not p.exists():
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Tier-A v1.2 residual train/eval (no diffusion)")
    ap.add_argument("--config", default="configs/tier_a_v1_3_joint.yaml")
    ...
    ap.add_argument(
        "--out-dir",
        default=None,
        help="Override cfg output_dir (e.g. runs/phase0/tier_a/v1_2b_thick2_train)",
    )
    ap.add_argument(
        "--ckpt",
        default=None,
        help="Frozen residual ckpt for --eval-only (default: v1_2b/best_residual.pt)",
    )
    args = ap.parse_args()
    ...
    # Protocol-locked thick-2 splits: ALWAYS prefer config year_split *_ids
    # (12/16/16). Manifest may list optional ic45 (SON balance) — do NOT let it
    # leak into v1.3 claim/eval echoes (Leonard 2026-09-15 promote WARN).
    splits = {
        "train": list(splits_cfg["train_ids"]),
        "val": list(splits_cfg["val_ids"]),
        "test": list(splits_cfg["test_ids"]),
    }
    if "ic45" in splits["test"]:
        raise SystemExit("FATAL: config year_split.test_ids must not include ic45 for v1.3")

    report: dict[str, Any] = {
        "schema": cfg.get("schema", "tier_a_v1_3_joint_results/v1"),
        "created_utc": utc_now(),
        "script": "code/tier_a/v1_3_joint/train.py",
        "config": str(cfg_path),
        "method": cfg.get("method"),
        "tier": "A",
        "variant": cfg.get("variant"),
        "cut": cfg.get("cut", "v1.2b-reg"),
        "diffusion": False,
        "fcn3_weights": "FROZEN",
        ...
        "honesty": cfg.get("notes", {}).get(
            "honesty",
            (
                "Tier-A v1.2b iterate FAIL v1.2 (w_120=2.0, patience=80) on expand pairs. "
    ...
    print(
        f"Tier-A {cfg.get('variant', '?')} cut={cfg.get('cut', 'v1.2b-reg')} diffusion=NO "
    ...
    if mode == "eval_only":
        ckpt_path = expand(
            args.ckpt
            or "~/fourcastnet/runs/phase0/tier_a/v1_2b_thick2_train/best_residual.pt"
        )
    ...
    gates = gate_pass_v1(
        ...
        raw_120,
        bars,
        n_eligible_saves=int(train_info.get("n_eligible_saves") or 0),
    ...
    report.update(
        {
            ...
            "holdout_expand": (
                cfg.get("holdout_expand")
                if cfg.get("holdout_expand") is not None
                else ("v1" if (mode == "eval_only" or uses_expand_bars(cfg)) else None)
            ),
            "provisional_years": bool(cfg["locks"].get("provisional_years", False)),
            ...
            "compare_v1_2": maybe_load_compare("v1_2", expand("~/fourcastnet/runs/phase0/tier_a/v1_2/tier_a_v1_2_results.json")),
            "gates": gates,
