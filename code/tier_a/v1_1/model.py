"""Smaller / regularized elevation-conditioned residual UNet for Tier-A v1.1.
...
RRCA-FD Plan A / Leonard TIER_A_V1_CALL.md (v1 FAIL iterate):
  ...
  - Do not overwrite v1/ (FAIL freeze).
"""
...
class ElevCondResidualUNet(nn.Module):
    ...
    def __init__(
        ...
        if depth < 2 or depth > 3:
            raise ValueError("v1.1 ElevCondResidualUNet supports depth 2 or 3")
        self.depth = depth
...
def build_model(cfg: dict) -> nn.Module:
    ...
    if name in {"PhysicsNeMoUNet", "physicsnemo_unet"}:
        raise RuntimeError(
            "PhysicsNeMo stock UNet is 3D; v1.1 uses ElevCondResidualUNet "
