#!/usr/bin/env python3
"""Tier-A v1.2b thick-2 RETRAIN — Leonard TIER_A_V1_2B_THICK2_TRAIN_RECIPE.md.

SCAFFOLD / Leonard FREEZE 2026-09-14 / Manisha greenlit GPU after dry-run.
Same ElevCondResidualUNet (base=16, dropout=0.20); w_120=2.0; patience=80; seed=42.
Pairs: holdout_expand=v2 (train 12 / val 16 / test 16). bars_set=thick2.
PASS LOCKED: n_eligible_saves >= 1; unconstrained fallback => auto FAIL.
FCN3 frozen. Writes ONLY under runs/phase0/tier_a/v1_2b_thick2_train/.
Does NOT overwrite v1_2b/, v1_2b_thick2/ (ZS), earlier frozen dirs, or CDS PID 611595.
No diffusion.
...
def artifact_stem(cfg: dict) -> str:
    return str(cfg.get("artifact_stem") or "tier_a_v1_2b_thick2_train")
...
def assert_no_overwrite(out_dir: Path, cfg: dict) -> None:
    """v1.2b thick-2 retrain writes ONLY under v1_2b_thick2_train/."""
    ...
    allowed = expand("~/fourcastnet/runs/phase0/tier_a/v1_2b_thick2_train").resolve()
    if not (out_r == allowed or str(out_r).startswith(str(allowed) + "/")):
        raise RuntimeError(
            f"v1.2b thick-2 retrain must write under runs/phase0/tier_a/v1_2b_thick2_train/ (got {out_r})"
        )
    for p in [
        expand("~/fourcastnet/runs/phase0/g0/g0_verifying_results.json"),
        expand("~/fourcastnet/runs/phase0/tier0_holdout/tier0_holdout_metrics.json"),
...
def main() -> int:
    ...
    ap.add_argument("--config", default="configs/tier_a_v1_2b_thick2_train.yaml")
    ...
    report: dict[str, Any] = {
        "schema": cfg.get("schema", "tier_a_v1_2b_thick2_train_results/v1"),
        "created_utc": utc_now(),
        "script": "code/tier_a/v1_2b_thick2_train/train.py",
        "config": str(cfg_path),
