#!/usr/bin/env python3
"""Tier-A v1-reg train/eval — larger elev-conditioned residual UNet.

Leonard TIER_A_V1_SKETCH_CALL.md: v1-reg FIRST. NO diffusion.
FCN3 frozen. Writes ONLY under runs/phase0/tier_a/v1/.
Does NOT overwrite v0/, v0_1/, g0_verifying, tier0_holdout.
...
Usage (from ~/fourcastnet):
  ...
  ~/fcn3-venv/bin/python code/tier_a/v1/train.py --config configs/tier_a_v1.yaml --train --epochs 400
"""
...
ROOT = HERE.parents[2]  # ~/fourcastnet  (code/tier_a/v1/)
...
def artifact_stem(cfg: dict) -> str:
    return str(cfg.get("artifact_stem") or "tier_a_v1")
...
def lead_weights(cfg: dict) -> dict[int, float]:
    """Per-lead loss weights. w_120 MUST be ≥ 1 (Leonard v1-reg freeze)."""
    t = cfg.get("train", {})
    ...
    if w120 < 1.0 - 1e-12:
        raise RuntimeError(f"w_120 must be >= 1 (got {w120}); Leonard TIER_A_V1_SKETCH_CALL")
...
def beat_this_echo(cfg: dict) -> dict:
    ...
    return {
        ...
        "val_120h_t2m_rmse_raw_le": float(bt.get("val_120h_t2m_rmse_raw_le", raw_val_120h_ref(cfg))),
        "val_120h_rule": bt.get(
            "val_120h_rule",
...
def gate_pass_v1(
    ...
    return {
        ...
        "note": (
            "v1-reg INTERIM PASS requires (1) val pooled < 1.948661 AND "
            "(2) test pooled < 1.850338 AND (3) val +120h t2m RMSE <= raw "
...
def assert_no_overwrite(out_dir: Path, cfg: dict) -> None:
    ...
    for p in forbidden:
        ...
        if p.is_file() and not p.exists():
            raise RuntimeError(f"forbidden artifact missing: {p}")
    v1 = expand("~/fourcastnet/runs/phase0/tier_a/v1")
    if out_r != v1.resolve() and not str(out_r).startswith(str(v1.resolve())):
        # allow only v1 output dir
        if "tier_a/v1" not in str(out_r):
            raise RuntimeError(f"v1-reg must write under runs/phase0/tier_a/v1/ (got {out_r})")
    for p in [
        expand("~/fourcastnet/runs/phase0/g0/g0_verifying_results.json"),
        expand("~/fourcastnet/runs/phase0/tier0_holdout/tier0_holdout_metrics.json"),
    ]:
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Tier-A v1-reg residual train/eval (no diffusion)")
    ap.add_argument("--config", default="configs/tier_a_v1.yaml")
    ...
    report: dict[str, Any] = {
        "schema": cfg.get("schema", "tier_a_v1_results/v1"),
        "created_utc": utc_now(),
        "script": "code/tier_a/v1/train.py",
        "config": str(cfg_path),
        ...
        "honesty": cfg.get("notes", {}).get(
            "honesty",
            (
                "Tier-A v1-reg diagnostic residual on frozen FCN3 crop → ERA5_interim. "
    ...
    print(
        f"Tier-A {cfg.get('variant', '?')} cut=v1-reg diffusion=NO "
